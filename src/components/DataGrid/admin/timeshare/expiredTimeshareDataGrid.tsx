...
function CustomNoRowsOverlay() {
  return (
    <StyledGridOverlay>
      ...
      <Box sx={{ mt: 1 }}>No Expired Timeshare Yet!</Box>
    </StyledGridOverlay>
  );
}

const ExpiredTimeshareDataGrid = () => {
  const navigate = useNavigate();
  const [expireTimeshareList, setExpireTimeshareList] = useState<
    AllTimeshare[]
  >([]);
  ...
  useEffect(() => {
    ...
    const initUseEffect = async () => {
      await getInActiveTimeshareList();
    };
    initUseEffect();
  }, []);
...
export default ExpiredTimeshareDataGrid;
